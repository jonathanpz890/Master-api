...
const LIST_FIELDS = [
    ...
    { name: 'Status', type: 'select', options: ['Todo', 'Doing', 'Done'], defaultValue: 'Todo', field: 'status' },
    { name: 'Priority', type: 'select', options: ['Low', 'Medium', 'High'], defaultValue: 'Medium', field: 'priority' },
...
export const getProjectById = async (req: Request, res: Response) => {
    try {
        ...
        if (project.taskList && (!project.taskLists || !project.taskLists.some((tl: any) => !tl.phaseId))) {
        ...
        if (!project.taskLists || !project.taskLists.some((tl: any) => !tl.phaseId)) {
            const list = await createTaskList(`${project.title} Tasks`, userId, project._id, project.color || '#8b5cf6');
        ...
        for (const phase of project.phases || []) {
            ...
            if (!hasPhaseList) {
                const list = await createTaskList(`${phase.title} Tasks`, userId, project._id, phase.color || project.color || '#8b5cf6');
...
export const createProject = async (req: Request, res: Response) => {
    try {
        ...
        const generalList = await createTaskList(`${newProject.title} Tasks`, userId, newProject._id, newProject.color || '#8b5cf6');
        ...
        for (const phase of newProject.phases || []) {
            const phaseId = phase._id?.toString();
            if (!phaseId) continue;
            const list = await createTaskList(`${phase.title} Tasks`, userId, newProject._id, phase.color || newProject.color || '#8b5cf6');
...
export const updateProject = async (req: Request, res: Response) => {
    try {
        ...
        if (updates.phases) {
            ...
            if (existingProject) {
                const existingPhaseIds = new Set((existingProject.taskLists || []).map((tl: any) => tl.phaseId));
                const newLists: any[] = [...(existingProject.taskLists || [])];

                for (const phase of updates.phases) {
                    const phaseId = phase._id?.toString();
                    if (phaseId && !existingPhaseIds.has(phaseId)) {
                        const list = await createTaskList(`${phase.title} Tasks`, userId, id, phase.color || existingProject.color || '#8b5cf6');
