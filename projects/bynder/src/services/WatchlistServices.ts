...
export const searchTMDB = async (req: Request, res: Response) => {
    try {
        ...
        if (!TMDB_API_KEY || TMDB_API_KEY === 'undefined') {
            res.status(400).json({ error: 'TMDB API Key is missing. Please add TMDB_API_KEY to your .env file.' });
            return;
        }
        if (!query) {
...
export const getExploreContent = async (req: Request, res: Response) => {
    try {
        ...
        console.error('TMDB Explore Error:', error.response?.data || error.message);
