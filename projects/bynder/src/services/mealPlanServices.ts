...
export const getMealPlansByRange = async (userId: string, startDate: Date, endDate: Date) => {
    ...
    }).populate('meals.recipeId').sort({ date: 1 });
};
...
export const updateMealInPlan = async (userId: string, date: string, mealId: string, update: any) => {
    const targetDate = new Date(date);
    targetDate.setUTCHours(0, 0, 0, 0);
...
export const moveMealRecord = async (userId: string, sourceDate: string, destDate: string, mealId: string, destCategory: string, sourceCategory?: string, destIndex?: number) => {
    try {
