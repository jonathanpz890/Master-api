...
import { parseRecipeFromInput, generateRecipeImage, parseRecipeFromImage, generateRecipeFromPrompt, generateRecipeConcepts } from "./aiService.js";
...
export const createRecipe = async (req: Request, res: Response) => {
    try {
        ...
        if (image) {
            imageUrl = `${process.env.SERVER_URL}/images/${image.filename}`;
        }
...
export const editRecipe = async (req: Request, res: Response) => {
    try {
        ...
        const { recipeId, title, servings, cookTime, prepTime, cookingMethod, ovenTemp, airFryTemp } = req.body;
        const ingredients = parseMultiformField(req.body.ingredients);
        const instructions = parseMultiformField(req.body.instructions);
        const category = req.body.category || 'Other';
        const image = req.file;
        ...
        if (image) {
            updateData.image = `${process.env.SERVER_URL}/images/${image.filename}`;
