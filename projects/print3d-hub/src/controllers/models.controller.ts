...
export const downloadModelFile = async (req: Request, res: Response) => {
  ...
  const allowedHost = url.hostname === 'thingiverse.com' || url.hostname.endsWith('.thingiverse.com');
  ...
  try {
    ...
    const response = await fetch(url, { headers: fetchHeaders, redirect: 'error', signal: controller.signal });
