...
const SENSITIVE_QUERY_KEYS = new Set(['token', 'password', 'secret', 'authorization', 'api_key', 'key', 'email', 'phone']);
...
export const requestLogger = (req: Request, res: Response, next: NextFunction): void => {
  ...
  res.once('finish', () => {
    ...
    if (res.statusCode >= 500) logger.error('HTTP request completed with server error', responseDetails);
    else if (res.statusCode >= 400) logger.warn('HTTP request completed with client error', responseDetails);
