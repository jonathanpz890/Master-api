...
export const getNotes = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Fetching notes failed', error);
...
export const createNote = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Creating note failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const updateNote = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Updating note failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const uploadNoteImage = async (req: Request, res: Response) => {
  try {
    ...
    const imageUrl = `${process.env.BYNDER_SERVER_URL}/images/${file.filename}`;
    ...
    logger.error('Uploading note image failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const uploadNoteDoodle = async (req: Request, res: Response) => {
  try {
    ...
    const doodleUrl = `${process.env.BYNDER_SERVER_URL}/images/${file.filename}`;
    ...
    logger.error('Uploading note doodle failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
