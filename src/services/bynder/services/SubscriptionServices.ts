...
export const getSubscriptions = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Fetching subscriptions failed', error);
...
export const createSubscription = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Creating subscription failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const updateSubscription = async (req: Request, res: Response) => {
  try {
    ...
    const subscription = await Subscription.findOneAndUpdate({ _id: id, userId }, updates, {
      new: true,
    });
    ...
    logger.error('Updating subscription failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const deleteSubscription = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Deleting subscription failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const getScanResults = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Fetching subscription scan results failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const bulkCreateSubscriptions = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Bulk creating subscriptions failed', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
