...
const refreshGoogleToken = async (user: IUser) => {
  ...
  try {
    const response = await axios.post('https://oauth2.googleapis.com/token', {
      client_id: process.env.BYNDER_GOOGLE_CLIENT_ID,
      client_secret: process.env.BYNDER_GOOGLE_CLIENT_SECRET,
...
export const scanEmailsForSubscriptions = async (userId: string, tokenOverride?: string) => {
  ...
  const query =
    'subscription OR receipt OR invoice OR payment OR renews OR renewal OR "order confirmation" OR bill OR statement';
  ...
  try {
    ...
    logger.error('Gmail subscription scan failed', error);
