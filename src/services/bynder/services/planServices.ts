...
const calculateProgress = (plan: any) => {
  ...
  if (type === 'Trip') {
    ...
    const basics = items.filter(
      (i: any) => !i.isHidden && !['flight', 'hotel', 'transport', 'pack'].includes(i.type),
    );
    ...
    const total =
      totalItems + totalFlights + totalAccommodations + totalTransportation + totalPacking;
    if (total === 0) return 0;
...
export const updatePlan = async (planId: string, update: any) => {
  ...
  if (update.data) {
    const currentData = plan.data
      ? typeof plan.data.toObject === 'function'
        ? plan.data.toObject()
        : plan.data
      : {};
