...
export const RECIPE_JSON_SCHEMA = `{
    ...
    "ingredients": [
        {
            "ingredient": {
                "name": "Ingredient name in the source language",
                "imageSearchName": "English singular ingredient name used only to find a Spoonacular image"
            },
...
export const RECIPE_FIDELITY_RULES = `
    ...
    3. SOURCE LANGUAGE: Preserve the source language exactly for the title, ingredient names, section headers, and instruction text. Do not translate these recipe fields into English.
    4. OUTPUT FORMAT: Return ONLY a JSON object matching this schema.
    5. INGREDIENT NAMES: Keep the displayed name in its original language and use a simple singular form where the source permits it. Keep quantities and hints in the amount/unit fields. Set imageSearchName to an English singular ingredient name solely for Spoonacular image lookup; it is internal metadata and must never replace the displayed ingredient name.
 ...
 */
export const buildParseFromTextPrompt = (content: string, imageHint: string, sourceLanguageHint?: string): string => `
    You are a meticulous recipe transcription specialist.
    ...
    10. LANGUAGE ENFORCEMENT: ${sourceLanguageHint
      ? `The source is ${sourceLanguageHint}. The title, ingredients, headings, and instructions MUST be returned in ${sourceLanguageHint}; translating them to English is incorrect.`
      : 'Detect the language used by the recipe source. The title, ingredients, headings, and instructions MUST stay in that same language; translating them to English is incorrect.'}
...
export const buildParseFromImagePrompt = (): string => `
    ...
    - Then, structure it into the JSON schema below while retaining the original language.
    - Do not translate the title, ingredients, headings, or instructions to English. English is permitted only in imageSearchName, which is hidden lookup metadata for Spoonacular thumbnails.
...
export const buildCreateFromPromptPrompt = (userInput: string, creativity: number): string => {
    ...
    3. LANGUAGE: Return the recipe in the same language as the user's request.
...
export const buildRecipeConceptsPrompt = (userInput: string, creativity: number): string => {
    ...
    6. LANGUAGE: Return concept titles and descriptions in the same language as the user's request.
