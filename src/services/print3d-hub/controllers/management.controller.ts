...
const signSession = (payload: string): string =>
  createHmac('sha256', requiredEnvironment('ADMIN_SESSION_SECRET'))
    .update(payload)
    .digest('base64url');
...
export const loginAdminController = async (req: Request, res: Response): Promise<void> => {
  ...
  if (
    ...
    constantTimeEquals(username, adminUser) &&
    constantTimeEquals(password, adminPass)
  ) {
    res.status(200).json({
      success: true,
      token: issueSession(),
...
export const addOrderController = async (req: Request, res: Response): Promise<void> => {
  ...
  const authoritativeModels = await Promise.all(models.map(async (model: any) => {
    ...
    if (
      ...
      job.fileKey !== model.fileKey
    )
      return null;
    const costs = rates[material];
    const unitPrice =
      7 + job.weightg * (costs.rate / costs.density) + (job.timeSeconds / 3600) * 0.9;
    return {
      ...
      material,
      color:
        typeof model.color === 'string' && /^#[0-9a-f]{6}$/i.test(model.color)
          ? model.color
          : '#111827',
      infill: job.infillDensity,
  ...
  const subtotal = Number(
    authoritativeModels.reduce((sum: number, model: any) => sum + model.price, 0).toFixed(2),
  );
...
export const addFilamentController = async (req: Request, res: Response): Promise<void> => {
  ...
  if (
    ...
    res
      .status(400)
      .json({ error: 'Material array, English name, Hebrew name, and Hex color are required' });
    return;
  }
...
export const uploadImageController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const configuredBaseUrl = process.env.BLUEPRINT_PUBLIC_BASE_URL?.replace(/\/$/, '');
...
export const addProductController = async (req: Request, res: Response): Promise<void> => {
  ...
  const slug = normalizeSlug(
    typeof req.body.slug === 'string' && req.body.slug ? req.body.slug : nameEn || '',
  );
  if (
    typeof nameEn !== 'string' ||
    !nameEn.trim() ||
    typeof nameHe !== 'string' ||
    !nameHe.trim() ||
    !slug
  ) {
    res.status(400).json({ error: 'English name, Hebrew name, and a valid slug are required' });
    return;
  }
  try {
    ...
    res.status(status).json({
      error: status === 409 ? 'A product with this slug already exists' : 'Failed to add product',
...
export const updateProductController = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(status).json({
      error:
        status === 409 ? 'A product with this slug already exists' : 'Failed to update product',
      message: error.message,
    });
  }
};
...
export const deleteProductController = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!(await DBService.deleteProduct(req.params.id))) {
      res.status(404).json({ error: 'Product not found' });
      return;
    }
...
export const getModelingRequestsController = async (
  _req: Request,
  res: Response,
): Promise<void> => {
  try {
...
export const addModelingRequestController = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!name || !email || !projectName || !description) {
    res
      .status(400)
      .json({ error: 'Missing required fields: name, email, projectName, description' });
    return;
  }
  try {
    const newRequest = await DBService.addModelingRequest({
      name,
      email,
      phone: phone || '',
      projectName,
...
export const updateModelingRequestStatusController = async (
  req: Request,
  res: Response,
): Promise<void> => {
  const { id } = req.params;
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: 'Failed to update modeling request status', message: error.message });
  }
};
...
export const trackEventController = async (req: Request, res: Response): Promise<void> => {
  ...
  if (
    ...
    (page !== undefined && (typeof page !== 'string' || page.length > 200)) ||
    (language !== undefined && (typeof language !== 'string' || language.length > 16)) ||
    (payload !== undefined &&
