...
export const sliceModelController = async (req: Request, res: Response): Promise<void> => {
  ...
  const validLayerHeights = [0.12, 0.2, 0.28];
  ...
  try {
    logger.info('Slicing requested', {
      filename: file.filename,
      size: file.size,
      material,
      infill: parsedInfill,
      layerHeight: parsedLayerHeight,
    });
    ...
    if (error.message === 'MODEL_TOO_LARGE') {
      res.status(400).json({
        ...
        message:
          'The model is too large to fit on the Bambu Lab P1S build volume (256mm x 256mm x 256mm). Please scale it down or split it.',
      });
      return;
    }
    ...
    if (error.message === 'SLICER_BUSY') {
      res
        .status(429)
        .json({ error: 'Slicer is busy', message: 'Please try again in a few minutes.' });
      return;
    }
