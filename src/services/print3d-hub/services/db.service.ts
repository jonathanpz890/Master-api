...
const DEFAULT_PRODUCTS: Omit<Product, 'id' | 'createdAt'>[] = [
  ...
  {
    slug: 'pocket-stash-box',
    nameEn: 'The Pocket Stash Box',
    nameHe: 'קופסת האחסון לכיס',
    descriptionEn: '',
    descriptionHe: '',
    categoryEn: 'Storage',
    categoryHe: 'אחסון',
  ...
  {
    slug: 'bedside-table-organizer',
    nameEn: 'Bedside Table Organizer',
    nameHe: 'מארגן לשידה ליד המיטה',
    descriptionEn: '',
    descriptionHe: '',
    categoryEn: 'Home organization',
    categoryHe: 'ארגון הבית',
    imageUrl: '',
    active: true,
    sortOrder: 4,
  },
  {
    slug: 'coffee-table-organizer-tissue-holder',
    nameEn: 'Coffee Table Organizer & Tissue Holder',
    nameHe: 'מארגן ושומר טישו לשולחן הקפה',
    descriptionEn: '',
    descriptionHe: '',
    categoryEn: 'Home organization',
    categoryHe: 'ארגון הבית',
  ...
  {
    slug: 'drawer-systems',
    nameEn: 'Drawer Systems',
    nameHe: 'מערכות מגירות',
    descriptionEn: '',
    descriptionHe: '',
...
export class DBService {
  public static async connectDB(): Promise<void> {
    const uri = process.env.BLUEPRINT_MONGODB_CONNECTION_URI;
    if (!uri) {
      throw new Error('BLUEPRINT_MONGODB_CONNECTION_URI must be configured');
    }
  ...
  public static async addOrder(order: Omit<Order, 'id' | 'status' | 'createdAt'>): Promise<Order> {
    ...
    if (order.models && Array.isArray(order.models)) {
      const tempUploadsDir = path.join(
        process.env.BLUEPRINT_TEMP_DIR ?? path.resolve(process.cwd(), 'data/print3d-hub/temp'),
        'uploads',
      );
      const permUploadsDir =
        process.env.BLUEPRINT_UPLOADS_DIR ??
        path.resolve(process.cwd(), 'data/print3d-hub/uploads');
      ...
      for (const model of order.models) {
        if (model.fileKey) {
          ...
          if (
            !tempFilePath.startsWith(`${path.resolve(tempUploadsDir)}${path.sep}`) ||
            !permFilePath.startsWith(`${path.resolve(permUploadsDir)}${path.sep}`)
          ) {
          ...
          if (fs.existsSync(tempFilePath)) {
            try {
              ...
              logger.error('Failed to persist sliced STL file', {
                fileKey: model.fileKey,
                error: err,
              });
            }
          } else {
            logger.warn('Sliced STL file not found in temporary storage', {
              fileKey: model.fileKey,
            });
          }
        }
  ...
  public static async updateOrderStatus(
    ...
    const doc = await OrderModel.findOneAndUpdate({ id }, { status }, { new: true }).populate<{
      customer: ICustomer;
    }>('customer');
    if (!doc) return null;
  ...
  public static async updateFilament(
    id: string,
    updates: Partial<Omit<Filament, 'id'>>,
  ...
  public static async addFilament(filament: Omit<Filament, 'id'>): Promise<Filament> {
    ...
    const cleanName = filament.nameEn
      ...
      .replace(/^_+|_+$/g, '');
    const matPrefix =
      filament.material && filament.material.length > 0
  ...
  public static async getOrderModelFile(orderId: string, fileKey: string): Promise<string | null> {
    ...
    const order = await OrderModel.findOne({ id: orderId, 'models.fileKey': fileKey }).select({
      models: 1,
    });
  ...
  public static async getProducts(activeOnly = false): Promise<Product[]> {
    const docs = await ProductModel.find(activeOnly ? { active: true } : {}).sort({
      sortOrder: 1,
      createdAt: 1,
    });
  ...
  public static async updateProduct(
    ...
    const doc = await ProductModel.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
    return doc ? this.mapProduct(doc) : null;
  ...
  public static async addGalleryItem(
  ...
  public static async addModelingRequest(
    data: Omit<ModelingRequest, 'id' | 'status' | 'createdAt'>,
  ): Promise<ModelingRequest> {
  ...
  public static async updateModelingRequestStatus(
    id: string,
    status: ModelingRequest['status'],
  ): Promise<ModelingRequest | null> {
  ...
  public static async getAnalyticsSummary(days: number = 30): Promise<any> {
    ...
    uniqueSessionsRaw.forEach((r: any) => {
      uniqueSessionsMap[r._id] = r.uniqueSessions;
    });
    ...
    const topModels = await AnalyticsEventModel.aggregate([
      ...
      {
        $group: {
          _id: { modelName: '$payload.modelName', thingId: '$payload.thingId' },
          opens: { $sum: 1 },
        },
      },
      { $sort: { opens: -1 } },
    ...
    const [modelingSubmitted, contactSubmitted] = await Promise.all([
      AnalyticsEventModel.countDocuments({
        eventType: 'modeling_request_submitted',
        createdAt: { $gte: since },
      }),
      AnalyticsEventModel.countDocuments({
